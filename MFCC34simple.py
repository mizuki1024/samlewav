...
def init_3d_plot(X, y, pca, templates):
    ...
    # グラフの設定（軸ラベルも入れ替え）
    ax.set_title("🎯 MFCC4-5母音空間（フォルマント情報に対応）", fontsize=16)
    ax.set_xlabel("MFCC5（第6係数）")  # X軸とY軸を入れ替え
    ax.set_ylabel("MFCC4（第5係数）")  # X軸とY軸を入れ替え
